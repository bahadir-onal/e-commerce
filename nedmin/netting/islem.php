...
if (isset($_POST['sepetsil'])) {
    ...
    $sil=$db->prepare("DELETE FROM sepet WHERE kullanici_id=:kullanici_id and urun_id=:urun_id");

    $kontrol = $sil->execute(array(

        'kullanici_id' => $_POST['kullanici_id'],
        'urun_id' => $_POST['sepetsil'],

    ));

    if ($kontrol) {

        header("Location:../../sepet.php?durum=sepetok");

    } else {

        header("Location:../../sepet.php?durum=sepetno");
